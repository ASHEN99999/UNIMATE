 ...
 * Accepts base64 encoded images and stores them
 */
const validateImages = (images: any[]): string[] => {
  ...
  return images.map((img, index) => {
    if (typeof img !== 'string') {
      throw new Error(`Image ${index + 1} must be a string`);
    }

    // Check if it's a valid base64 string
    if (!img.startsWith('data:image/')) {
      throw new Error(`Image ${index + 1} must be a valid base64 encoded image`);
    }

    // Optional: Check image size (base64 is ~33% larger than binary)
    // Max ~5MB binary = ~6.67MB base64
    if (img.length > 7 * 1024 * 1024) {
      throw new Error(`Image ${index + 1} exceeds size limit`);
    }
...
export const createItem = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    // Validate and process images
    const processedImages = validateImages(images);
