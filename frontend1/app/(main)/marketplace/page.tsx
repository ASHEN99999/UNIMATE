...
export default function MarketplacePage() {
  ...
  useEffect(() => {
    const fetchItems = async () => {
      try {
        setIsLoading(true)
        const data = await secondhandService.getAllItems()
        setItems(data)
      } catch (err: any) {
        console.error("Error fetching marketplace items:", err)
        setError("Failed to load items. Showing mock data instead.")
        setItems(mockSecondHandItems as any)
      } finally {
        setIsLoading(false)
      }
    }
  ...
  const filteredItems = useMemo(() => {
    return items.filter((item) => {
      ...
      if (item.isActive === false || item.status === "sold") return false
        ...
        <TabsContent value="browse" className="mt-6 space-y-6">
          ...
          {isLoading ? (
            <div className="flex flex-col items-center justify-center py-20">
              <Spinner className="h-10 w-10 text-[oklch(0.50_0.15_145)] mb-4" />
              <p className="text-muted-foreground">Fetching marketplace listings...</p>
            </div>
